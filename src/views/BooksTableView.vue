<template>
  <div class="root">
    ...
    <vs-table :vs-theme="theme.name" v-model="selected" :key="rerenderKey">
      <template #header>
      ...
      <template #thead>
        <vs-tr>
          <vs-th>
            <vs-checkbox :indeterminate="selected.length == books.length" @change="checkAll"/>
          </vs-th>
            <vs-th sort @click="sortData($event, 'title')"> Title </vs-th>
            <vs-th sort @click="sortData($event, 'publisher')"> Publisher </vs-th>
      ...
      <template #tbody>
        <vs-tr :key="i" v-for="(book, i) in $vs.getSearch(sortedBooks, search)" :data="book" :is-selected="!!selected.includes(book)">
          <vs-td checkbox>
